...
change must use the named gpt-local tool. Never invent, recycle, or infer a run
suffix. After create_workspace returns, use its complete, unredacted
workspace_id as <run-id> in every path, operation ID, and idempotency key.
Redact it only in the final human-readable report.
...
2. Call list_projects, then create one detached workspace for project
   "Gpt-Local" with task_name "section6-live-chatgpt". Explicitly omit
   idempotency_key from create_workspace so a previous run can never be
   replayed. Record workspace_id and worktree_path, then set
   <run-id>=<the complete returned workspace_id>. Call git_status and save the
   complete initial status as initial_status. Require no dirty entries. The test
   path is "section6-live-<run-id>.txt".

3. Begin Change Set A with explanation "section6 live atomic composition" and
   idempotency_key="begin-a-<run-id>". Record change_set_id, revision,
   before_tree_hash, base_head, and expires_at. Immediately call get_change_set
   and require status="open", revision=1, no operations, and no changed files.
...
4. Prove path rejection without contaminating staging. Call stage_replace on
   ...
   new_text="y", explanation="must be denied", operation_id="deny-<run-id>",
   and idempotency_key="deny-<run-id>". Require error.code="PATH_DENIED".
...
5. Stage a new UTF-8 file with stage_patch on Change Set A. Use this exact
   unified diff after replacing <run-id>:

   --- /dev/null
   +++ b/section6-live-<run-id>.txt
   @@ -0,0 +1 @@
   +section6-alpha

   Use explanation="create isolated acceptance file",
   operation_id="create-<run-id>", and idempotency_key="create-<run-id>".
...
7. Stage ordered replacement on Change Set A:
   ...
   operation_id="replace-<run-id>", idempotency_key="replace-<run-id>").
...
9. Prove validation invalidation. Stage another replacement on Change Set A:
   ...
   explanation="invalidate prior validation", operation_id="gamma-<run-id>",
   idempotency_key="gamma-<run-id>". Require ordinal=3, revision=4, and
   ...
   idempotency_key="stale-commit-<run-id>". Require
...
10. Revalidate Change Set A with expected_revision=4. Record
    ...
    idempotency_key="commit-a-<run-id>". Require status="committed" and
...
11. Call rollback_change_set on committed Change Set A with
    idempotency_key="rollback-committed-<run-id>". Require
    error.code="CHANGE_SET_ALREADY_COMMITTED" and verify the test file still
    contains "section6-gamma\n".

12. Prove whole-workspace conflict detection. Begin Change Set B with
    idempotency_key="begin-b-<run-id>". Stage replace on the test path from
    "section6-gamma" to "section6-staged-conflict" using operation_id and
    idempotency_key "conflict-stage-<run-id>", then validate its returned
    revision. Then make one explicitly permitted ordinary replace_text call on
    the real workspace, replacing "section6-gamma" with
    "section6-external-change", explanation="create Change Set conflict",
    idempotency_key="external-<run-id>". Attempt commit_change_set B using its
    validated digest and before tree with idempotency_key="commit-b-<run-id>".
...
13. Roll back Change Set B with idempotency_key="rollback-b-<run-id>". Require
...
14. Restore cleanliness entirely through Change Sets. Begin Change Set C with
    idempotency_key="begin-c-<run-id>" and stage this exact deletion patch after
    replacing <run-id>:

    --- a/section6-live-<run-id>.txt
    +++ /dev/null
    @@ -1 +0,0 @@
    -section6-external-change

    Validate the returned revision and commit with
    idempotency_key="commit-c-<run-id>". Require the test path no longer exists.
...
16. If and only if every assertion passed, call discard_workspace with
    idempotency_key="discard-section6-<run-id>" and require removed_path=true and
