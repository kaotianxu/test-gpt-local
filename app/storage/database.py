...
def init_db(db_path: str | Path | None = None) -> None:
    ...
    conn.executescript("""
        ...
        CREATE INDEX IF NOT EXISTS idx_change_set_operations_ordinal
            ON change_set_operations(change_set_id, ordinal);
    """)
    workspace_columns = {
...
def delete_workspace(workspace_id: str) -> bool:
    ...
    with _DB_LOCK:
        try:
            ...
            conn.execute(
                """DELETE FROM change_set_operations WHERE change_set_id IN
                   (SELECT change_set_id FROM change_sets WHERE workspace_id=?)""",
                (workspace_id,),
            )
            conn.execute(
                """DELETE FROM change_set_files WHERE change_set_id IN
                   (SELECT change_set_id FROM change_sets WHERE workspace_id=?)""",
                (workspace_id,),
            )
            conn.execute("DELETE FROM change_sets WHERE workspace_id=?", (workspace_id,))
            cursor = conn.execute(
